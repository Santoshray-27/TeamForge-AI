...
export const HackBot: React.FC = () => {
  ...
  const sendMessage = async (text?: string) => {
    ...
    try {
      ...
      // chatWithHackBot handles all errors internally with fallback responses
    } finally {
      setLoading(false);
    }
  };
      ...
      <AnimatePresence>
        {!isOpen && (
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={handleOpen}
            className="fixed bottom-6 right-6 z-50 w-14 h-14 rounded-2xl bg-gradient-to-br from-indigo-600 to-purple-600 flex items-center justify-center shadow-2xl shadow-indigo-500/40 cursor-pointer"
          >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-6 right-6 z-50 w-[380px] max-w-[calc(100vw-24px)] rounded-2xl overflow-hidden shadow-2xl shadow-black/50 border border-white/10"
          ...
          >
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {QUICK_PROMPTS.map((prompt) => (
                        <button
                          ...
                          disabled={loading}
                          className="text-xs px-2.5 py-1.5 bg-white/5 hover:bg-indigo-600/20 border border-white/10 hover:border-indigo-500/40 rounded-lg text-gray-400 hover:text-indigo-300 transition-all disabled:opacity-50"
                        >
                          {prompt}
                        </button>
                      ))}
                  ...
                  <div className="flex gap-2 items-end">
                    <div className="flex-1 relative">
                      <input
                        ...
                        disabled={loading}
                        className="w-full px-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-white placeholder-gray-600 text-sm focus:outline-none focus:border-indigo-500/50 transition-all disabled:opacity-50"
                      />
                    </div>
                    <motion.button
                      ...
                      className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-600 to-purple-600 flex items-center justify-center disabled:opacity-40 disabled:cursor-not-allowed flex-shrink-0 cursor-pointer"
                    >
                      {loading ? (
            ...
            {isMinimized && (
              <div
                className="bg-[#0f0f1a] px-4 py-2 flex items-center gap-2 cursor-pointer border-t border-white/5"
